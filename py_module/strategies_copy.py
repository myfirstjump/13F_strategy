...
class Strategy13F(object):
        ...
        for idx, hedge_fund in enumerate(hedge_fund_list):
            ...
            for idx_q, (quarter, holdings_time, filing_number) in enumerate(zip(quarters_list, date_list, filing_list)):
                ...
                if idx_q > 0: #扣除第一季，每季要計算的內容
                    ...
                    merged_data = self.shares_difference_between_quarters____(previous_holdings, holdings_data)
                    '''
                    ...
                    query = self.create_query_get_open_price___(sym_str, holdings_time)
                    # print(query)
    ...
    def create_query_get_open_price(self, SYMs_tuple, date, fund, quarter, filing_number):
        ...
        return query
    def create_query_get_open_price___(self, SYMs_tuple, date):
        '''
        依據holdings去查表price，透過stock_id(即holdings表中的SYM) join兩張表格，並加入SHARES資訊至price表。
        '''
        price_table = self.us_stock_price_table
        query = ''' SELECT [date], [stock_id] SYM, [Open]
            FROM {} WITH(NOLOCK) WHERE [stock_id] IN {}
            AND [date] = '{}'
            '''.format(price_table, SYMs_tuple, date)
        return query
    ...
    def shares_difference_between_quarters____(self, previous_holdings, holdings_data):
        scaling_in = {}
        scaling_out = {}
        scaling_even = {}
